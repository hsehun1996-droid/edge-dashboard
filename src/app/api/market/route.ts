...
export async function GET() {
  ...
  try {
    ...
    const indices: MarketIndex[] = INDICES.map((meta, i) => {
      const result = results[i]

      if (result.status === "rejected" || !result.value) {
        // Fallback: seeded deterministic mock data
      ...
      // 장중: daily bars에 오늘 종가가 아직 없으므로 validCloses[-1] = 전날 종가
      // 장외: validCloses[-1] = 오늘(최근) 종가, validCloses[-2] = 전날 종가
    ...
    return NextResponse.json({ data: indices, timestamp })
  } catch (err) {
